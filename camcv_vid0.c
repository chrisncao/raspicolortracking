...
// 
//
// Changed to track color objects by Ning - Sept. 2013
// ncao@ualberta.ca
//
/////////////////////////////////////////////////////////////
...
//new 
int nCount=0;
IplImage *py, *pu, *pv, *pu_big, *pv_big, *image,* dstImage, *imageMarker, *imageHSV ;
 ...
 *
 * @param source image pointer
 * @return binary image pointer 
 *
 *  The function follows OpenCV tutorial - Color Detection & Object Tracking  
 *  http://opencv-srf.blogspot.ro/2010/09/object-detection-using-color-seperation.html
 */

IplImage* GetThresholdedImage(IplImage* imgHSV){       
    IplImage* imgThresh=cvCreateImage(cvGetSize(imgHSV),IPL_DEPTH_8U, 1);
    cvInRangeS(imgHSV, cvScalar(90,120,40,0), cvScalar(130,255,255,0), imgThresh);   ////////// track blue object
    return imgThresh;
}
 ...
 *  This function threshold the HSV image and create a binary image
 *
 * @param binary image pointer
 * @coords of center are saved in lastX, lastY
 *
 *  The function follows OpenCV tutorial - Color Detection & Object Tracking  
 *  http://opencv-srf.blogspot.ro/2010/09/object-detection-using-color-seperation.html
 */
...
void trackObject(IplImage* imgThresh){
	...
	// if the area<1000, it's treated as noise
	if(area>1000){
		...
		// draw a filled circle at the center of gravity
		cvCircle(imageMarker, cvPoint(posX, posY), 2, cvScalar(0,0,255,0), -1, 8, 0);
   ...
   if (pData)
   {
      ...
      if (buffer->length)
      {
		...
		if (pData->pstate->graymode==0)
		{
			...
			cvCvtColor(dstImage,imageHSV, CV_BGR2HSV); // convert to HSV color space for obj detection
			IplImage *imageThresh = GetThresholdedImage(imageHSV);
			trackObject(imageThresh);
			cvAdd(dstImage,imageMarker,dstImage,NULL);
			
			cvShowImage("camcvWin", dstImage );
			cvShowImage("binary",imageThresh );
			cvReleaseImage(&imageThresh);
		}
		else // useless for color tracking
		{	
...
int main(int argc, const char **argv)
{
	...
	imageHSV = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3); 
	imageMarker = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3); 
	cvZero(imageMarker);
